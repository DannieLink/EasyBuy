	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				%>
				<tr>
					<td class="first w4 c"><%=u.getEu_user_id() %></td>
					<td class="w1 c"><%=u.getEu_user_name() %></td>
					<td class="w2 c"><%=u.getEu_sex() %></td>
					<td><%=u.getEu_email() %></td>
					<td class="w4 c"><%=u.getEu_mobile() %></td>
					<td class="w4 c"><%=u.getEu_money() %></td>
					<td class="w1 c">
					<%if(user.getEu_status()==2){ %>
					    <a href="manage/user-modify.jsp?id=<%=u.getEu_user_id() %>">修改</a>
					    <a href="UpdateUserServ?type=delete&id=<%=u.getEu_user_id() %>">删除</a>
					 <%}else{ %>
					 	<a href="manage/user-modify.jsp?id=<%=u.getEu_user_id() %>">修改</a>
					 <%} %>
					 </td>
			...
			<div class="pager">
				<ul class="clearfix">
					...
					<li><a href="SearchUserById?id=<%=user.getEu_user_id() %>&pageNo=1">首页</a>&nbsp;</li>
					<li>
						<c:if test="${pageBean.currentNo!=1}">
						<a href="SearchUserById?id=<%=user.getEu_user_id() %>&pageNo=${pageBean.currentNo-1}">上一页</a>&nbsp;
						</c:if>
						<c:if test="${pageBean.currentNo==1}">
					...
					<li>
						...
						<a href="SearchUserById?id=<%=user.getEu_user_id() %>&pageNo=${pageBean.currentNo+1}">下一页</a>&nbsp;
						</c:if>
						<c:if test="${pageBean.currentNo==pageBean.totalPages}">
						<a href="javascript:alert('当前页已是最后一页')">下一页</a>&nbsp;
						</c:if>
					</li>
					<li><a href="SearchUserById?id=<%=user.getEu_user_id() %>&pageNo=${pageBean.totalPages}">末页</a>&nbsp;</li>
