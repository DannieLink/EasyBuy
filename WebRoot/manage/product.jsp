	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<%for(Product p:list){%>
				<tr>
					<td class="first w4 c"><%=p.getEp_id() %></td>
					<td class="thumb"><a href="product-view.jsp?id=<%=p.getEp_id() %>" target="_self"><%=p.getEp_name() %></a></td>
					<td class="w1 c"><a href="manage/product-modify.jsp?id=<%= p.getEp_id()%>">修改</a> <a class="manageDel" href="DeleteProductSer?id=<%=p.getEp_id() %>">删除</a></td>
				</tr>
				<%} %>
			</table>
			 <div class="pager">
				<ul class="clearfix">
					...
					<li><a href="SelectProductBackSer?pageNo=1">首页</a>&nbsp;</li>
					<li>
						<c:if test="${pageBean.currentNo!=1}">
						<a href="SelectProductBackSer?pageNo=${pageBean.currentNo-1}">上一页</a>&nbsp;
						</c:if>
						<c:if test="${pageBean.currentNo==1}">
					...
					<li class="current">
						...
						<a href="SelectProductBackSer?pageNo=${pageBean.currentNo+1}">下一页</a>&nbsp;
						</c:if>
						<c:if test="${pageBean.currentNo==pageBean.totalPages}">
						<a href="javascript:alert('当前页已是最后一页')">下一页</a>&nbsp;
						</c:if>
					</li>
					<li><a href="SelectProductBackSer?pageNo=${pageBean.totalPages}">末页</a>&nbsp;</li>
