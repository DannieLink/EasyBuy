	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<%ArrayList<ProductType> list = (ArrayList<ProductType>)request.getAttribute("list"); %>
				<%for(ProductType pt:list){%>
				<tr>
					<td class="first w4 c"><%=pt.getEpc_id() %></td>
					<td><%=pt.getEpc_name() %></td>
					<td class="w1 c"><a href="manage/productClass-modify.jsp?id=<%=pt.getEpc_id()%>&name=<%=pt.getEpc_name() %>">修改</a> <a class="manageDel" href="DeleteTypeSer?id=<%=pt.getEpc_id() %>">删除</a></td>
				</tr>
				<%} %>
			 ...
			 <div class="pager">
				<ul class="clearfix">
					...
					<li><a href="TypeManageSer?pageNo=1">首页</a>&nbsp;</li>
					<li>
						<c:if test="${pageBean.currentNo!=1}">
						<a href="TypeManageSer?pageNo=${pageBean.currentNo-1}">上一页</a>&nbsp;
						</c:if>
						<c:if test="${pageBean.currentNo==1}">
					...
					<li class="current">
						...
						<a href="TypeManageSer?pageNo=${pageBean.currentNo+1}">下一页</a>&nbsp;
						</c:if>
						<c:if test="${pageBean.currentNo==pageBean.totalPages}">
						<a href="javascript:alert('当前页已是最后一页')">下一页</a>&nbsp;
						</c:if>
					</li>
					<li><a href="TypeManageSer?pageNo=${pageBean.totalPages}">末页</a>&nbsp;</li>
