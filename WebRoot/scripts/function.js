...
function reloadCode(){
//换验证码
var src=  $("#safeCode").attr("src");

if(src=="Number.jsp"){
 $("#safeCode").attr("src","Number2.jsp");
}else{
$("#safeCode").attr("src","Number.jsp");
}
...
//            location.href = "Cart?action=addAddress&address="+$("#addAddr").val();
    ...
    $(".number").find("span").click(function(){
        ...
        if(opr=="del"){
        	...
        	//找到库存数量
        	 var to=$(this).parent().find("div").text();
        	if(number<parseInt(to)){
            number++;
        	}else{
        	alert('不得大于库存数量！');
        	}
    ...
    $("#logout").click(function(){
        if(confirm("确定注销吗？")) {
            location.href="RemoveUserServ";
        }else{
            location.href="SelectAllProductSer";
            return false;
        }
    });
  //轮换广告
...
//    })
      ...
      $("#product .thumb img").mouseover(function(){
        var src= $(this).attr("src").split("/");
        var name=src[src.length-1].split(".")[0];
        $("#product").append('<img id="bigImg" src="images/product/'+name+'.jpg"  width="300" height="300"/>');
        $("#bigImg").show().error(function(){
            $(this).remove();
        });
    }).mouseleave(function(){
